...
def calculate_atreus_62_original_pcb_offset_to_match_position_of_case(
    ...
    edge_cut_lines = list(
        filter(
            ...
            kicad.get_collection(atreus_62_board_data, "gr_line"),
        )
    )

    midpoint_of_pbc_for_x_offset = flip_point_over_x_axis(
        midpoint(
            kicad.get_values(edge_cut_lines[5], "start"),
            kicad.get_values(edge_cut_lines[14], "start"),
        )
    )
    ...
    y_offset = upper_right_hand_footprint_center[1] - upper_right_hand_switch_center[1]

    x_offset = midpoint_of_pbc_for_x_offset[0] - case_geometry["mirror_at"]["point"][0]
